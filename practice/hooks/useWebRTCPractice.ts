...
export const useWebRTCPractice = ({ localStream }: UseWebRTCPracticeParams) => {
  ...
  const createPeerConnection = () => {
    ...
    try {
      ...
      const pc = new RTCPeerConnection({ iceServers: ICE_SERVERS }); // 상대방과 연괄하는 통로
      ...
      // pc: 배송트럭, 이통로에 내 미디어를 상대방에게 보낼께요 라고 등록
      localStream
        ?.getTracks()
  ...
  // ═════════════════════════════════════════════════════════════════════
  // ✍️ TODO 3: 연결 제안 생성 (기존 참가자가 신규 참가자에게)
  // ═════════════════════════════════════════════════════════════════════
  const createConnectionProposal = async () => {
    ...
    try {
      ...
      await pc.setLocalDescription(offerObj); // 내가 보낼/받을 수 있는 미디어 정보를 내 PeerConnection에 공식적으로 등록하는 것"
      // 여기에 코드를 작성하세요!
  ...
  // ═════════════════════════════════════════════════════════════════════
  // ✍️ TODO 4: 연결 제안 수락 (신규 참가자가 받아서 Remote Description 설정)
  // ═════════════════════════════════════════════════════════════════════
  const acceptConnectionProposal = async (offerText: string) => {
    const pc = pcRef.current;
    if (!pc) {
      console.error("❌ PeerConnection이 없습니다. 먼저 생성하세요.");
      return;
    ...
    try {
      ...
      // 상대방이 보낼 수 있는 미디어 등록햇어->그 정보를 받을 준ㅂ완료! =>상대방의 제안을 받아서 PeerConnection에 등록"
  ...
  // 상대반 제안에 대한 내답변 생성 및 등록
  const sendConnectionResponse = async () => {
    const pc = pcRef.current;
    if (!pc) {
  ...
  // ═════════════════════════════════════════════════════════════════════
  // ✍️ TODO 5: 연결 응답 적용 (기존 참가자가 신규 참가자의 Answer 받기)
  // ═════════════════════════════════════════════════════════════════════
  const applyConnectionResponse = async (answerText: string) => {
    const pc = pcRef.current;
    if (!pc) {
      console.error("❌ PeerConnection이 없습니다.");
      return;
